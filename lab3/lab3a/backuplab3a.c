...
void superblock_parser()
{
  ...
  if ((int)(my_superblock->s_inodes_count / my_superblock->s_inodes_per_group) != (int)(my_superblock->s_blocks_count / my_superblock->s_blocks_per_group))
    {
      fprintf(stderr, "Superblock - group inconsistency: blocks * blocks per group: %d, inodes*inodes per group: %d\n", 
	      my_superblock->s_blocks_count / my_superblock->s_blocks_per_group,
              my_superblock->s_inodes_count / my_superblock->s_inodes_per_group);
       exit(1);
    }
...
void bitmap_parser()
{
  ...
  for (i = 0; i < group_num; i++)
      ...
      for (j = 0; j < my_superblock->s_log_block_size; j++)
	{
      ...
      for (j = 0; j <my_superblock->s_inodes_per_group/8/*my_superblock->s_log_block_size*/; j++)
	{
		  ...
		  //if (/*(j*8+k+1) <= (my_superblock->s_inodes_per_group)*/1) //the inode number cannot exceed the inodes per group
		      ...
		      else
			{
		      ...
		      fprintf(inode_fd,"%o,%d,%d,%d,%x,%x,%x,%d,%d,",my_inode->i_mode,
			      ...
			      my_inode->i_atime,
 			      my_inode->i_size,
			      my_inode->i_blocks /(( my_superblock->s_log_block_size)/512));
		      for (m = 0; m < 14; m++)
			{
...
void indirect_parser()
{
  ...
  for (i = 0; i < indirect_offset_counter; i++)
    {
      uint32_t indirect1;
      uint32_t indirect2;
      uint32_t indirect3;
